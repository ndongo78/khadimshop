...
const Cart = () => {
  ...
  return (
    <Grid
      className={cart.length === 0 ? classes.container1 : classes.container}
    >
      <Typography
        variant="h3"
        color="primary"
      ...
      >
        ...
        {cart.length === 0
          ? "Le panier est vide"
          : "Voici les articles ajoutés au panier"}{" "}
      </Typography>
      {cart.map((item) => (
        <Container
          maxWidth="md"
          style={{ padding: 15 }}
          className={classes.conteMild}
          key={item.id}
        >
          <Paper elevation={3} className={classes.boxs}>
            ...
            <Box>
              ...
              <Typography
                ...
                color="primary"
              >
                Prix: {item.price * item.qty} CFA{" "}
              </Typography>
              <Paper elevation={0} component="div">
                <IconButton
                  className={classes.moins}
                  onClick={() => dispatch(removeItem(item.id))}
                >
                  {" "}
                  <Remove />{" "}
                </IconButton>
                <Paper elevation={0} component="span" className={classes.qty}>
                  {" "}
                  {item.qty}{" "}
                </Paper>
                <IconButton
                  className={classes.add}
                  onClick={() => dispatch(addCartItems(item.id))}
                >
                  {" "}
                  <Add />{" "}
                </IconButton>
      ...
      {cart.length > 0 && (
        <Box className={classes.recap}>
          <Paper className={classes.recapBt}>
            ...
            <Button className={classes.btn} onClick={handleRedirect}>
              {" "}
              Payer ma commande{" "}
            </Button>
          </Paper>
